...
// Start JsSIP Definition Stuff
// Modified BHK Code start Here - from Chris Code
// document.addEventListener("DOMContentLoaded", ready);

JsSIP.debug.enable("JsSIP:*");

var socketSip = new JsSIP.WebSocketInterface("ws://192.168.31.18:5066");

var webRTCMedia = document.getElementById("webRTCMedia");

var userAgent = new JsSIP.UA({
  sockets: socketSip,
  uri: "1003@192.168.31.18",
  password: "4124450711",
...
var options = {
  ...
  extraHeaders: [
    "Subject: <CallTreatment>Conference</CallTreatment>" +
      "<passcode>" +
      "1234567" +
      "</passcode>"
  ]
...
$('#sip-call').click(function() {
    alert('We are making a call');
    // We call what was entered in the input
    window.webRTCSession = userAgent.call($('#extn').val(), options);

    window.webRTCSession.connection.addEventListener("track", e => {
        streamSip = e.streams[0];
        webRTCMedia.srcObject = streamSip;
        webRTCMedia.play();
      });
...
$('#start-button').click(function () {
    ...
    /*
    // Did get the audio from user media, but we want to get it from the JsSIP stream
...
let streamAudioToWebSocket = function (userMediaStream) {
    ...
    // micstreams emit a format event - doesn't look like SIP stream dows this, so this doesn't work
